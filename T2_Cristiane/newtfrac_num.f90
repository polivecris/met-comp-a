program newtfrac
  ...
  do I=0, N     ! Varre parte real, ou seja, x                                                                                                         
     ...
     do J=0, N     ! Varre parte imaginária, ou seja, y                                                                                                
        ...
        do L=1, Nmax
	df= (f(z+dz)-f(z))/dz
           if (df == 0) then     ! Derivada igual a zero                                                                                             
              exit

           else
              z = z - (f(z)/df)     ! Método de Newton-Raphson                                                                                       
              if (abs(f(z)) < e) then     ! Zero da função                                                                                             
                 print*, "I",I,"J",J, "raiz", z
                 if (I==0 .and. J==0) then     ! Primeira raiz                                                                                         
                    RAIZ(1)=z; NR = 1; write(filename,'(A1,I2.2,A4)') "r",NR,".dat"; open(NR,file=filename)
                 end if
                 ...
                 do K=1, NR
                    if (abs(z - RAIZ(K)) < e) then     ! Raízes são iguais                                                                             
                       flag = .false.; write(filename,'(A1,I2.2,A4)') "r",K,".dat"; write(K,*)x,y
                       exit
		       ...
		       flag = .true.
                 ...
                 if(flag) then     ! Armazena as raízes diferentes                                                                                     
                    NR=NR+1; RAIZ(NR)=z; write(filename,'(A1,I2.2,A4)') "r",NR,".dat"; open(NR,file=filename); write(NR,*)x,y
                 end if
                 exit
              end if
           end if
